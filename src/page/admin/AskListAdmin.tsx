...
import { APIAnswerAsk, APIAskListAdmin, APIDeleteAsk, APIStoreAsk } from '../../api/AskAPI';
...
function AskListAdmin() {
  ...
  const getAskList = async () => {
    ...
    const { list, total } = await APIAskListAdmin(data);
  ...
  const onStoreAsk = async (idx: number) => {
    const data = {
      idx: idx,
    };
    try {
      const res = await APIStoreAsk(data);
      console.log(res);
  ...
  return (
    <>
                ...
                <Accordion.Item key={item.idx} className={classes.item} value={String(item.idx)}>
                  <Accordion.Control>
                    <TitleBox>
                      ...
                      <RowWrap last>
                        <RowWrap>
                          <Nickname>{item.user ? item.user.nickname : '-'}</Nickname>
                          <Line />
                          <Date>{dayjs(item.created_time).format('YYYY.MM.DD. HH:mm:ss')}</Date>
                        </RowWrap>
                        <UnderlineTextButton onClick={() => onStoreAsk(item.idx)}>보관</UnderlineTextButton>
                      </RowWrap>
                    </TitleBox>
                  </Accordion.Control>
                  <Accordion.Panel>
      ...
      <AlertModal
        ...
        setVisible={setShowStoreModal}
        onClick={() => {
          setShowStoreModal(false);
        }}
        text="1:1 문의가 보관 처리되었습니다."
      />
      <AlertModal
